<?php 
    ...
    if($rSel == 1){
        $sqlUp = "UPDATE member SET mId='$mId' WHERE email = '$email'";
        $qUp = $n->inUp($sqlUp);
        if($qUp == 1){
            $_SESSION['iUser'] = $qSel;
            echo json_encode("1");
    ...
    if($rSel == 0){
        ...
        $sqlIn = "INSERT INTO member (memId,mId,memShort,nickName,email,memPic,memberDate,nCompany) VALUES          
                  ('$memId','$mId','$memShort','$displayName','$email','$pictureUrl','$date','$nCompany')";
        $qIn = $n->inUp($sqlIn);
        $qSel2 = $n->sel($sqlSel);
